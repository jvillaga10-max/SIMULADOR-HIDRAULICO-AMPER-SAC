...
# MOTOR INTERNO DE ACCESORIOS (Base Crane)
LE_D_CRANE = {
...
if "df_tramos" not in st.session_state:
    st.session_state.df_tramos = pd.DataFrame({
        ...
        "Tees (Dir)": [0, 0, 0, 0, 0],
        "Yees (Dir)": [0, 0, 0, 0, 0],
        "Yees (Ramal)": [0, 0, 0, 0, 0],
        "Valv. Compuerta": [0, 0, 0, 0, 0],
        "Valv. Mariposa": [0, 0, 0, 0, 0]
    })
...
def calcular_hidraulica(q_s, df_config, tc_amortiguado):
    ...
    for _, r in df_config.iterrows():
        try: l_total += float(r.get("Longitud(m)", 0.0))
        except: pass
...
if len(res_tramos) > 0:
    ...
    c_btn.download_button(
        ...
        data=export_data,
        file_name="Memoria_Calculo_AMPER.xlsx",
